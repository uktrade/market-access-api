...
class BarrierReport(ArchivableMixin, BaseModel):
    ...
    name = models.CharField(
        max_length=128, help_text="User defined report name", null=True
    )
    status = models.CharField(
        choices=BarrierReportStatus.choices, default=BarrierReportStatus.PENDING
    )
    user = models.ForeignKey(
        User, related_name="barrier_reports", on_delete=models.SET_NULL, null=True
    )
    filename = models.CharField(max_length=settings.CHAR_FIELD_MAX_LENGTH)

    def processing(self):
        if self.status != BarrierReportStatus.PENDING:
            raise BarrierReportStatusUpdateError(
                "Can only process a pending Barrier Report"
            )
    ...
    def complete(self):
        if self.status != BarrierReportStatus.PROCESSING:
            raise BarrierReportStatusUpdateError(
                "Can only complete a processing Barrier Report"
            )
