...
def convert_null_to_empty(apps, schema_editor):
    ...
    HistoricalPublicBarrierNote = apps.get_model(
        "interactions", "HistoricalPublicBarrierNote"
    )
    ...
    HistoricalInteraction.objects.filter(archived_reason__isnull=True).update(
        archived_reason=""
    )
    HistoricalInteraction.objects.filter(text__isnull=True).update(text="")
    HistoricalInteraction.objects.filter(documents_cache__isnull=True).update(
    ...
    PublicBarrierNote.objects.filter(archived_reason__isnull=True).update(
        archived_reason=""
    )
    HistoricalPublicBarrierNote.objects.filter(archived_reason__isnull=True).update(
        archived_reason=""
    )
