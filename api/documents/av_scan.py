...
from api.documents.exceptions import VirusScanException
from api.documents.utils import get_document_by_pk
...
def _download_and_scan_file(document_pk: str, download_url: str):
    ...
    logger.critical("----------------------------------------------")
    logger.critical("About to virus scan request to url:")
    logger.critical(settings.AV_V2_SERVICE_URL)
    logger.critical("----------------------------------------------")
...
def _multipart_encoder(document_pk, content, content_type):
    multipart_fields = {"file": (document_pk, content, content_type)}
