...
class Barrier(FullyArchivableMixin, BaseModel):
    ...
    def save(
        ...
        for field in non_editable_public_fields:
            ...
            setattr(public_barrier_instance[0], field, internal_value)
        public_barrier_instance[0].categories.set(self.categories.all())
        public_barrier_instance[0].save()
        public_barrier_instance[0].unpublished_changes
...
class PublicBarrier(FullyArchivableMixin, BaseModel):
    ...
    def unpublished_changes(self):
        ...
        if changed_list != []:
            ...
            if self.changed_since_published is not True:
                self.changed_since_published = True
                self.save()
        else:
            if self.changed_since_published is not False:
