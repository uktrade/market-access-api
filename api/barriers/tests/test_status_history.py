...
class TestBarrierStatusHistory(APITestMixin):
    ...
    def test_barrier_priority_history(self):
        list_report_url = reverse("list-reports")
        list_report_response = self.api_client.post(
            list_report_url,
            format="json",
        ...
        assert response.data["history"][1]["new_value"] == "2"
        assert response.data["history"][1]["field_info"]["event"] == "BARRIER_CREATED"
        assert response.data["history"][1]["field_info"]["status_date"] is not None
        assert response.data["history"][1]["date"] is not None
        assert response.data["history"][2]["old_value"] == "None"
        assert response.data["history"][2]["new_value"] == "MEDIUM"
        assert response.data["history"][2]["field_info"]["priority_summary"] == "some priority summary"
        assert response.data["history"][2]["field_info"]["priority_date"] is not None
        assert response.data["history"][2]["date"] is not None