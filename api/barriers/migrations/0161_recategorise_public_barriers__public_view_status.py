...
def recategorise_public_barrier_statuses(apps, schema_editor):
    ...
    """
    PublicBarrierModel = apps.get_model(
...
def undo_recategorisation(apps, schema_editor):
    ...
    HistoricalPublicBarrierModel = apps.get_model(
        "barriers", "HistoricalPublicBarrier"
    )

    awaiting_approval_barriers = PublicBarrierModel.objects.filter(_public_view_status=70)
    for public_barrier in awaiting_approval_barriers:
        history = HistoricalPublicBarrierModel.objects.filter(barrier_id=public_barrier.barrier_id).order_by('history_date')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("barriers", "0160_alter_historicalpublicbarrier__public_view_status_additions"),
    ]

    operations = [
        migrations.RunPython(recategorise_public_barrier_statuses, reverse_code=undo_recategorisation),
    ]
