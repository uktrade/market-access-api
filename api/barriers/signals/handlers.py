...
def related_barrier_update_embeddings(sender, instance, *args, **kwargs):
    logger.info(f'(Handler) Running related_barrier_update_embeddings() handler for {instance.pk}')
    try:
    ...
    logger.info(f"(Handler) Updating related barrier embeddings for {instance.pk}: {changed}")
