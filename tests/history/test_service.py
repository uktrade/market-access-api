...
from api.history.v2.service import get_model_history, convert_v2_history_to_legacy_object
...
def test_programme_fund_history_no_history(barrier):
    ...
    assert get_model_history(qs, model="test", fields=fields, track_first_item=True) == []
...
def test_model_history_tracking_first_item(barrier):
    ...
    qs = ProgrammeFundProgressUpdate.history.filter(barrier__id=barrier.id)
    fields = ("milestones_and_deliverables", "expenditure")
    model_history = get_model_history(qs, model="test", fields=fields, track_first_item=True)
...
def test_model_history_multiple_items(barrier):
    obj = ProgrammeFundProgressUpdateFactory(barrier=barrier, expenditure="A", milestones_and_deliverables="AA")
    ...
    assert ProgrammeFundProgressUpdate.objects.filter(barrier=barrier, id=obj.id).count() == 1
    assert ProgrammeFundProgressUpdate.history.filter(barrier=barrier, id=obj.id).count() == 4  # 1 create + 3 updates
    ...
    assert model_history == [
        ...
        },
        {
            ...
            "fields": {
                "milestones_and_deliverables": {"new": "BB", "old": "AA"}
            },
            'user': {'id': None, 'name': None}
        },
        {
...
def test_convert_to_legacy_object(barrier):
    obj = ProgrammeFundProgressUpdateFactory(barrier=barrier, expenditure="1", milestones_and_deliverables="arsenal")
    ...
    assert legacy_representation[1].data == {
        'date': model_history[0]["date"],  # double model change so shares date of previous
        'field': 'expenditure',
        'model': 'test',
        'new_value': '1',
        'old_value': None,
