...
def test_urls():  # noqa: D403
    ...
    with override_settings(ROOT_URLCONF="tests.documents.my_entity_document.urls"):
        yield
...
class TestDocumentViews(APITestMixin):
    ...
    def test_documents_list(self, test_urls):
        ...
        entity_document = MyEntityDocument.objects.create(
            my_field="cats have five toes", original_filename="test.txt"
        )
        ...
        entity_document_to_be_deleted = MyEntityDocument.objects.create(
            my_field="now what?", original_filename="test1.txt"
        )
    ...
    def test_document_retrieval(self, test_urls):
        ...
        entity_document = MyEntityDocument.objects.create(
            original_filename="test.txt", my_field="cats are lactose intolerant"
        )

        url = reverse(
            "test-document-item", kwargs={"entity_document_pk": entity_document.pk}
        )
    ...
    def test_document_with_deletion_pending_retrieval(self, test_urls):
        ...
        entity_document = MyEntityDocument.objects.create(
            original_filename="test.txt", my_field="large field"
        )
    ...
    def test_document_schedule_virus_scan(self, virus_scan_document, test_urls):
        ...
        url = reverse(
            ...
            kwargs={"entity_document_pk": entity_document.pk},
        )
    ...
    def test_document_delete(self, delete_document, test_urls):
        ...
        entity_document = MyEntityDocument.objects.create(
            original_filename="test.txt", my_field="cats can recognise human voices"
        )
