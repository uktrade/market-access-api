...
def barrier_factory(users):
    current_financial_year_start = datetime(datetime.now().year, 4, 1)

    def func(date=None):
        if not date:
            date = FuzzyDate(
                start_date=current_financial_year_start,
                end_date=current_financial_year_start + timedelta(days=45),
